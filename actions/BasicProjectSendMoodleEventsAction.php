    ...
    protected function responseProcess() {
        ...
        if(isset($this->params["moodleToken"])){
            ...
            if($courseId){
                ...
                $response['info'] = $this->generateInfo("info", WikiIocLangManager::getLang('MOODLE_EVENTS_HAS_BEEN_UPDATED'), $id);
            }else{
                $response['info'] = $this->generateInfo("error", WikiIocLangManager::getLang('INVALID_COURSE_ID'), $id);
                $response['info'] = $this->addInfoToInfo($response['info'], WikiIocLangManager::getLang('MOODLE_EVENTS_NOT UPDATED'), $id);            
            }
        }else{
            $response['info'] = $this->generateInfo("error", WikiIocLangManager::getLang('USER_IS_NOT_MOODLE_USER'), $id);
            $response['info'] = $this->addInfoToInfo($response['info'], WikiIocLangManager::getLang('MOODLE_EVENTS_NOT UPDATED'), $id);            
        }
        return $response;
    }
