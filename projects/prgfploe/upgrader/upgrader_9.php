<?php
/**
 * upgrader_9: Transforma el archivo continguts.txt de los proyectos 'prgfploe'
...
class upgrader_9 extends ProgramacionsCommonUpgrader {
    ...
    public function process($type, $ver, $filename=NULL) {
        switch ($type) {
            ...
            case "templates":
                ...
                $aTokMov = [['regexp0' => "<WIOCCL:SET var=\"keyUfPAF\".*\n<WIOCCL:SET var=\"nombreEACs\".*\n",
                ...
                $aTokRep = [["(^La Qualificació Final de la Unitat Formativa.*?següent)( sempre.*?)(:)",
                             '$1<WIOCCL:IF condition="{#_SUMA({#_SUMA({##notaMinimaEAF##},{##notaMinimaJT##})_#},{##notaMinimaPAF##})_#}\\>0">$2</WIOCCL:IF>$3']
                           ];
                $dataChanged = $this->updateTemplateByReplace($dataChanged, $aTokRep);

                $aTokRep = [["(^<WIOCCL:IF condition=\"\{##keyUfJT##\}===false && \{##keyUfEAF##\}===false)(\">)",
                             "$1 && {##notaMinimaPAF##}\\>0$2"]
                           ];
                $dataChanged = $this->updateTemplateByReplace($dataChanged, $aTokRep);
