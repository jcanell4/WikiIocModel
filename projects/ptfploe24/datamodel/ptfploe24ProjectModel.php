...
//            $unTable[$key]["ordreImparticio"] = $originalufTable[$key]["ordreImparticio"];
    ...
    public function updateCalculatedFieldsOnSave($data, $originalDataKeyValue=FALSE, $subset=FALSE) {
        ...
        if (!empty($taulaCalendari) && !empty($taulaDadesUn)){
            ...
            $horesUn = array();
            $horesUn[0] = 0;
            //Marjose: si hi ha hores de calendari per una unitat, aquestes són les que es registren
            //i trepitjen les hores que hi ha a $taulaDadesUn. 
            //si no hi ha hores de calendari per una unitat, $hores estarà buit, el que es registra
            //seran les hores de $taulaDadesUn
            for ($i=0; $i<count($taulaDadesUn); $i++){
                $idU = intval($taulaDadesUn[$i]["unitat"]);               
                if (isset($hores[$idU])){
                    $taulaDadesUn[$i]["hores"]=$hores[$idU];
                }else{
                    $horesUn[$idUn]=0;
                }
            ...
            if($resultatsAprenentatge){ //aqui revisar per afegir ponderacio?
            ...
            if (!empty($taulaDadesUn)){
                for ($i=0; $i<count($taulaDadesUn); $i++){
                    ...
                    if (isset($horesUn[$idUn])){
                        $taulaDadesUn[$i]["hores"] = $horesUn[$idUn];
                    }
            ...
            //$values["taulaDadesUnitats"] = $taulaDadesUnitats;//aqui revisar si cal mantenir o canviar per taulaDadesRA
            $values["taulaDadesUn"] = $taulaDadesUn;
        }
...
//                        error_log("D0.3.- A punt d'actualitzar.");
    ...
    public function validateFields($data = NULL, $subset=FALSE){
        if($subset!==FALSE && $subset!=ProjectKeys::VAL_DEFAULTSUBSET){
            ...
            // Comprovació de la correspondència entre "unitat" i "bloc"
            $verifica = false;
            $bloc = array_search($values['tipusBlocModul'], ["mòdul","1r. bloc","2n. bloc","3r. bloc"]);
            if (empty($bloc)) $bloc = 0;
            foreach ($taulaDadesUn as $un) {
                $verifica |= ($un['bloc'] == $bloc);
            }
            if (!$verifica && !empty($taulaDadesUn)) {
                throw new InconsistentDataException("No hi ha cap unitat que pertanyi al bloc ({$taulaDadesUn['bloc']})[$bloc] definit en aquest pla de treball");
            }
            ...
            if (!empty($values["nsProgramacio"])){
                ...
                for ($i=0; $i<count($taulaDadesUn); $i++){
                    ...
                    $horesU[$idU]+= $taulaDadesUn[$i]["hores"]; 
                }
